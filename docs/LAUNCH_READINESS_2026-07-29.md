...
지금 트래픽이 없으므로 **색인을 여는 이득이 사실상 0**이다. 두 갈래 중 하나를 고르면 된다.

| 선택 | 내용 | 비용 |
| --- | --- | --- |
| **도메인 연결까지 색인 차단**(권장) | robots를 한시적으로 전면 Disallow. 도메인 붙이고 `NEXT_PUBLIC_SITE_URL`을 바꾼 뒤 연다 | 코드 한 줄, 되돌리기 쉬움 |
| 지금부터 색인 | 색인 나이를 조금 먼저 쌓는다 | 도메인 연결 시 전 URL 이전 + 리다이렉트 |
