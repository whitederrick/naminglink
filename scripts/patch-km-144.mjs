// 배치 144(밀러 `Ointment`~`Opium`) — 기존 둘을 손본다.
//
//   carriage(마차) ← 밀러 `Omnibus` 하나. 원문 자신이 「[141] See Carriage」라 적었다
//   laudanum(아편) ← 밀러 `Opium` 하나. 「아편」이 이미 그 상징의 이름이다
//
// **한 번만 돌린다.** 실행: node scripts/patch-km-144.mjs
...
const PATCHES = {
  carriage: {
    aliasesAdd: ["합승마차"],
    contextsAdd: {
      "합승마차를 타고 거리를 지남": "합승마차 거리를 지나 오해",
    },
    contextsEnAdd: {
      "합승마차를 타고 거리를 지남": "omnibus streets misunderstandings promises",
    },
  },
  laudanum: {
    aliasesAdd: [],
    contextsAdd: {
      "아편 꿈을 꿈": "낯선 이들이 교활한 홀리는",
    },
    contextsEnAdd: {
      "아편 꿈을 꿈": "strangers obstruct sly seductive",
    },
  },
};
let changed = 0;
