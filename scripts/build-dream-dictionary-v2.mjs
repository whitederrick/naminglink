...
const FALLBACK_FIRST = new Map([
  ...
  ["letter", "편지를 봉함"],
  // **바꾼 것**(2026-09-03 배치 100) — 밀러가 이 상징의 **가장 일반적인 그림**을 들여왔다.
  // 옛 기본값 「번갯빛이 몸을 비춤」은 조건이 붙은 좁은 자리라, 막연한 「번개 꿈」이
  // 떨어질 자리는 「번개를 봄」이다(배치 54의 `hunting`·`embrace` 와 같은 판단).
  ["lightning", "번개를 봄"],
  ["lion", "사자를 봄"],
  // **바꾼 것**(2026-09-03 배치 125) — 옛 기본값 「남과 함께 젖을 먹음」은 조건이 붙은
  // 자리이고, 밀러의 첫 문장 「젖을 마심」이 가장 일반적인 그림이다.
  ["milk", "젖을 마심"],
  // **바꾼 것**(2026-09-03 배치 124) — 있던 다섯이 전부 조건이 붙은 그림이고,
  // 밀러 `Mice` 의 첫 문장이 이 상징의 가장 일반적인 그림이다.
  ["mouse", "쥐를 봄"],
  // **바꾼 것**(2026-09-03 배치 127) — 옛 기본값 「장례 설교를 하려고 목사를 부름」은
  // 좁은 자리이고, 밀러 `Minister` 의 첫 문장이 가장 일반적인 그림이다.
  ["clergyman", "목사를 봄"],
  // **그대로 얼린 것**(2026-09-03 배치 127) — 밀러 `Mire` 가 앞으로 정렬되지만
  // 옛 「진흙 속에 빠져 있음」이 더 일반적인 그림이다.
  ["mud", "진흙 속에 빠져 있음"],
  // **바꾼 것**(2026-09-03 배치 129) — 옛 기본값 둘은 좁은 자리이고,
  // 밀러 `Monk`·`Monkey` 의 첫 문장이 그 상징의 가장 일반적인 그림이다.
  ["monk", "스님을 봄"],
  ["monkey", "원숭이를 봄"],
  // **바꾼 것**(2026-09-03 배치 130) — 옛 「남에게 돈과 물건을 돌려줌」은 좁은 자리이고,
  // 막연한 돈 꿈이 떨어질 자리는 「돈이 많이 있는 것을 봄」이다.
  ["money", "돈이 많이 있는 것을 봄"],
  // **그대로 얼린 것**(2026-09-03 배치 130) — `m130.json` 이 `m40.json` 보다 앞서 정렬되지만
  // 옛 「황금을 손에 쥠」(밀러 `Gold` 의 To handle gold)이 더 일반적인 그림이다.
  ["gold", "황금을 손에 쥠"],
  // **바꾼 것**(2026-09-03 배치 131) — 막연한 달 꿈이 떨어질 자리는
  // 밀러의 「하늘이 여느 때와 같은데 달을 봄」이다.
  ["moon", "하늘이 여느 때와 같은데 달을 봄"],
  // **그대로 얼린 것**(2026-09-03 배치 131) — `m131.json` 이 `m24.json`·`m47.json` 보다
  // 앞서 정렬되지만(`1` < `2`·`4`) 옛 기본값이 더 일반적인 그림이다.
  ["daybreak", "날이 밝아 오는 것을 봄"],
  ["hide", "짐승 가죽을 봄"],
  // **바꾼 것**(2026-09-03 배치 132) — 옛 「모기가 사람을 묾」은 주공해몽의 좁은 그림이고,
  // 막연한 모기 꿈이 떨어질 자리는 밀러의 「모기를 봄」이다.
  ["mosquito", "모기를 봄"],
  // **바꾼 것**(2026-09-03 배치 133) — 옛 「나방이 등불로 날아듦」은 좁은 그림이고
  // 밀러의 「나방을 봄」이 막연한 나방 꿈이 떨어질 자리다.
  ["moth", "나방을 봄"],
  // **그대로 얼린 것**(2026-09-03 배치 134) — 밀러 `Mountain` 다섯이 앞으로 정렬되지만
  // 다섯 다 조건이 붙은 그림이라 기본값 자리에 어울리지 않는다.
  ["mountain", "산이 해와 달을 물고 있음"],
  // **바꾼 것**(2026-09-03 배치 135) — 밀러의 「오디를 봄」이 막연한 오디 꿈이 떨어질 자리다.
  ["mulberry", "오디를 봄"],
  // **그대로 얼린 것**(2026-09-03 배치 135) — 밀러 쪽은 조건이 붙은 그림이다.
  ["mule", "노새를 타고 감"],
  // **그대로 얼린 것**(2026-09-03 배치 136) — 밀러 쪽은 조건이 붙은 그림
  // (「고운 가락」·「민물 홍합」)이라 기본값 자리에 어울리지 않는다.
  ["music", "남이 풍악을 울림"],
  ["clam", "조개 꿈을 꿈"],
  // **그대로 얼린 것**(2026-09-03 배치 137) — 밀러 `Mute` 쪽은
  // 「말 못 하는 이와 이야기를 나눔」이라는 좁은 그림이다.
  ["dumb", "말을 못 하게 됨"],
  // **그대로 얼린 것**(2026-09-03 배치 138) — 밀러 `Naked` 일곱이 앞으로 정렬되지만
  // 옛 「옷 없이 몸을 드러냄」이 막연한 알몸 꿈이 떨어질 자리다.
  ["naked-body", "옷 없이 몸을 드러냄"],
  // **바꾼 것**(2026-09-03 배치 139) — 옛 「바늘을 얻음」(주공해몽)은 좁은 자리이고,
  // 밀러의 「바늘을 씀」이 바늘 꿈의 가장 흔한 그림(바느질)이다. 얼린 판별어는
  // 「썼다 사용했 바느질했」로 좁혀 남은 넷과 부딪치지 않는다.
  ["needle", "바늘을 씀"],
  // **바꾼 것**(2026-09-03 배치 139) — 옛 「빈 새 둥지를 봄」은 좁은 자리(빈 둥지)이고,
  // 밀러의 「새 둥지를 봄」이 둥지 꿈의 가장 막연한 그림이다. 얼린 판별어는
  // 「새 둥지를 사업에 관심」으로 좁혀 남은 셋과 부딪치지 않는다.
  ["bird-s-nest", "새 둥지를 봄"],
  // **바꾼 것**(2026-09-03 배치 140) — 옛 「길을 가다 어둠이 덮침」은 걷는다는 조건이
  // 붙은 자리이고, 밀러의 「밤에 둘러싸임」이 어둠 꿈의 가장 막연한 그림이다.
  ["dark", "밤에 둘러싸임"],
  // **그대로 얼린 것**(2026-09-03 배치 140) — `m140.json`이 기존 파일보다 앞서 정렬되지만
  // 옛 「몸에 통증이 있음」이 더 일반적인 그림이다(새 「저림」은 그 한 갈래다).
  ["ache", "몸에 통증이 있음"],
  // **그대로 얼린 것**(2026-09-03 배치 140) — 옛 「숫자를 봄」이 더 일반적인 그림이다
  // (새 「숫자 때문에 사업이 어수선함」은 조건이 붙은 좁은 자리다).
  ["figure", "숫자를 봄"],
  // **그대로 얼린 것**(2026-09-03 배치 140) — 옛 「피를 흘리는 꿈을 꿈」이 더 일반적인
  // 그림이다(새 「코피가 남」은 코라는 부위가 붙은 좁은 자리다).
  ["bleeding", "피를 흘리는 꿈을 꿈"],
  // **바꾼 것**(2026-09-03 배치 142) — 옛 「강과 바다가 넘쳐 불어남」(주공해몽)은 홍수라는
  // 조건이 붙은 자리이고, 밀러의 「바다가 잔잔함」이 바다 꿈의 가장 막연한 그림이다.
  ["sea", "바다가 잔잔함"],
  // **그대로 얼린 것**(2026-09-04 배치 143) — 밀러 `Offense`(모욕당했다고 느낌)가
  // 앞으로 정렬되지만 옛 `Affront`의 「모욕당하는 꿈을 꿈」이 더 막연한 그림이다.
  ["humiliation", "모욕당하는 꿈을 꿈"],
  // **그대로 얼린 것**(2026-09-04 배치 143) — 밀러 `Office`(관직을 지님)가 앞으로
  // 정렬되지만 옛 주공해몽 「새로 벼슬을 받음」이 조건 없는 더 막연한 그림이다.
  ["official-post", "새로 벼슬을 받음"],
  // **그대로 얼린 것**(2026-09-04 배치 143) — 밀러 `Offspring`(제 자식을 봄)이 앞으로
  // 정렬되지만 옛 「아름다운 아이들을 여럿 봄」을 그대로 둔다 — 어느 쪽도 명백히 더
  // 넓지 않아 기존 화면을 흔들지 않는 쪽을 골랐다.
  ["child", "아름다운 아이들을 여럿 봄"],
  // **바꾼 것**(2026-09-04 배치 143) — 옛 「여성이 새끼를 거느린 가금을 봄」은 성별과
  // 가금이라는 조건이 붙은 자리이고, 밀러 `Offspring`의 「집짐승의 새끼를 봄」이
  // 새끼 꿈의 가장 막연한 그림이다.
  ["brood", "집짐승의 새끼를 봄"],
  // **바꾼 것**(2026-09-04 배치 144) — 옛 「아편을 먹음」은 행위가 붙은 자리이고,
  // 밀러 `Opium`의 「아편 꿈을 꿈」이 아편 꿈의 가장 막연한 그림이다.
  ["laudanum", "아편 꿈을 꿈"],
  // **바꾼 것**(2026-09-04 배치 145) — 옛 「집안에서 재물을 나눔」은 나눈다는 행위가
  // 붙은 좁은 자리이고, 밀러 `Opulence`의 「참된 재물과 안락을 누림」이 더 막연하다.
  ["wealth", "처녀가 참된 재물과 안락을 누림"],
  // **그대로 얼린 것**(2026-09-04 배치 145) — 밀러 `Orchard`가 앞으로 정렬되지만
  // 옛 주공해몽 「과수원 속을 거닒」이 정인·꽃이라는 조건이 없어 더 막연하다.
  ["orchard", "과수원 속을 거닒"],
  // **그대로 얼린 것**(2026-09-04 배치 146) — 밀러 `Ottoman`이 앞으로 정렬되지만
  // 옛 「긴 소파에 기대어 누움」이 정인이라는 조건이 없어 더 막연하다.
  ["couch", "긴 소파에 기대어 누움"],
  // **바꾼 것**(2026-09-04 배치 147) — 옛 「남의 외투를 입음」은 남의 옷이라는 조건이
  // 붙은 자리이고, 밀러 `Overcoat`의 「외투 꿈을 꿈」이 외투 꿈의 가장 막연한 그림이다.
  ["coat", "외투 꿈을 꿈"],
  // **바꾼 것**(2026-09-04 배치 148) — 옛 「누런 소가 집으로 옴」(주공해몽)은 색과
  // 「집으로 옴」이라는 조건이 붙은 자리이고, 밀러 `Ox`의 「잘 먹여 살진 소를 봄」이
  // 소 꿈의 가장 막연한 그림이다.
  ["ox", "잘 먹여 살진 소를 봄"],
  // **그대로 얼린 것**(2026-09-03 배치 133) — 밀러 `Mourning` 이 앞으로 정렬되지만
  // 옛 「몸에 상복을 입음」이 더 일반적인 그림이다.
  ["mourning-clothes", "몸에 상복을 입음"],
  ["jolly", "벗들과 흥겹게 어울림"],
  ["melon", "오이 꿈을 꿈"],
  ["beggar", "늙고 쇠약한 거지를 봄"],
  ["eating", "혼자서 먹음"],
  ["mattress", "요를 펴서 깖"],
  ["beetles", "몸에 딱정벌레가 있는 것을 봄"],
  ["carnival", "카니발에 어울려 참여함"],
  ["straw-mat", "돗자리를 집 안으로 들임"],
  ["bog", "늪지 꿈을 꿈"],
  ["wedding", "혼인하는 것을 봄"],
  ["house", "집을 지음"],
  ["headgear", "값진 머리쓰개를 봄"],
  // 좁은 쪽(여성이 ~)을 추출 파일 앞에 두어 동점을 풀었다 — 기본값은 옛 값 그대로 얼린다
  ["man", "남자와 몸을 섞음"],
  ["gavel", "의사봉을 봄"],
  ["judge", "판사 앞에 서게 됨"],
  ["lute", "품속에 비파가 있음"],
  ...
  ["lice", "머릿니가 나오는 꿈을 꿈"],
  ["engine", "기관차를 봄"],
  // **바꾼 것**(2026-09-03 배치 107) — 있던 넷이 전부 조건이 붙은 그림이고,
  // 밀러 `Locust` 의 첫 문장이 이 상징의 가장 일반적인 그림이다.
  ["grasshopper", "메뚜기를 봄"],
  ["burden", "무거운 짐을 짊어짐"],
  ["bread", "여성이 빵을 먹는 꿈을 꿈"],
