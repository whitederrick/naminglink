...
- 전 상품 판매 중지 상태에서는 요금 금액과 도장 신청서를 노출하지 않는다. 부분 판매 시 한
  ...
  필터와 회귀 검사를 보완한다.
- **운영 브라우저 실측은 끝났다(2026-08-11).** 화면 64곳(로케일 8 × 경로 8)에서 광고 요청
  0건이었고, `ko`의 진짜 결과에서는 요청이 나갔다. ALL PASS · exit 0.
- 다시 확인할 때는 손으로 열지 말고 **`node scripts/verify-no-ad-requests.mjs`**를 쓴다
  (인자 없음. 부르는 법이 궁금하면 `node scripts/audit-verifiers.mjs --list`).
  - 없어야 함: 로그인·요금·주문·가이드·404·빈 결과·비한국어 화면
  - **있어야 함**: `ko` + 실제 복원 결과 + 유효 슬롯. 「0건」만 세면 **제대로 껐다**와
    **배선이 죽었다**가 같은 모양이 된다
  - 화면이 **기대한 응답 코드로 떴는지**까지 본다. 경로가 틀리면 「광고 요청 0건」으로
    조용히 통과하던 자리다
