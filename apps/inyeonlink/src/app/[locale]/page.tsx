...
export async function generateMetadata({
  ...
  // **주소에 로케일이 없는 랜딩은 이제 없다.** 무접두 `/`는 미들웨어가 언어판으로 보내므로
  // 여기까지 오는 것은 언제나 `/{locale}`이고, 그 언어판이 스스로의 canonical이다.
...
export default async function LandingPage({
   ...
   * "가치가 별로 없는 콘텐츠" 반려를 받고 넣은 자리다 — 히어로 한 장뿐이면 문서 아홉 편이
   * 있어도 홈에서 그 존재를 알 길이 없어 크롤러·이용자 모두에게 "내용 없는 사이트"로 보인다.
   *
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="mx-auto w-full max-w-7xl px-5 py-14 sm:px-8 lg:px-10">
        <div className="max-w-3xl">
          <p className="text-sm font-semibold tracking-wide text-brand-plum">
            {guideDoc.eyebrow}
          </p>
        ...
        {/* 자료 숫자판. 값은 DB·엔진에서 오고, 못 읽으면 `DocBody`가 판을 통째로 뺀다. */}
        {statsSection ? (
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-2">
          {guideCards.map((card) => (
            <Link
              ...
              dir={textDirection}
            >
              <p className="text-xs font-semibold tracking-wide text-brand-plum">
                {card.doc.eyebrow}
              </p>
        ...
        <Link
          ...
          className="group mt-3 grid gap-1 rounded-lg border border-brand-plum/25 bg-surface-strong px-5 py-4 transition hover:border-foreground"
          dir={textDirection}
        >
          <p className="text-xs font-semibold tracking-wide text-brand-plum">
            {aboutDoc.eyebrow}
