...
function Block({
  ...
  if ("figure" in block) {
    ...
    const drawing =
      block.figure === "branch-wheel" ? (
        <>
          <BranchWheel labels={block.labels} />
          <BranchWheelLegend labels={block.labels} />
        </>
      ) : block.figure === "four-pillars" ? (
        <FourPillarsDiagram labels={block.labels} />
      ) : null;
