...
export const zh: Dictionary = {
  ...
  form: {
    ...
    intro: "Inyeon-Link 为每个人构建自己的八字，然后评分两个轴。四柱相合检查四个方面——每个日主在十神中的关系、五行是否能满足对方的需求、是否有任何一方的命盘在配偶位置（正财或正官），以及日支是否相合或相冲——而生肖相合仅查看年份支。两个轴通过加权平均结合在一起，即使没有出生时间，解读也能进行。\n这里使用的每个权重和规则都在下面的推理文档中发布。",
    personA: "我的信息",
    personB: "我的缘分对象",
    nickname: "姓名（本名）",
  ...
  affinity: {
    ...
    formIntro: "缘分可以在没有对方出生日期的情况下进行。它反向运行兼容性引擎——尝试缺失方可能持有的每个值——并在四个相同的轴上找到最适合你的类型：日主关系、五行补充、日支关系和配偶位置。它单独显示每个轴，而不是将它们合并为一个分数，因为在真实的人身上，日主和元素并不是独立移动的，因此将每个轴的最佳结果拼接在一起会描述出一个实际上不存在的人。\n相同的推理也适用于四柱命理的兼容性。",
    meLegend: "我",
    genderHint:
