 ...
 * `ReportPurchasePanel.tsx`(클라이언트 컴포넌트)가 못 부른다 — `locale.ts`를 `locale-
 * detect.ts`에서 갈라 둔 것과 같은 이유다. 예전에는 이 함수 자체를 그대로 복붙해 두 벌
 * 두었는데(2026-08-26 코드 리뷰에서 발견), 규칙을 한쪽만 고치면 화면이 보여 준 가격과
 * 서버가 만드는 주문이 어긋나고 타입이 같아서 컴파일러도 못 잡는 자리였다. 순수 함수라
 * `server-only`가 필요 없으므로 이 작은 파일로 뽑아 양쪽이 가져다 쓴다.
 *
