...
if (files.length === 0) {
   ...
   *
   * `verify-legal-source.ts` 가 세운 계약을 따른다 — 출력의 **마지막 줄**에 `CANNOT_RUN` 을 적고 0이 아닌
   * 코드로 끝낸다. 감사기는 그 토큰을 읽어 **「못 돎」**으로 가른다. 못 돎은 통과가 아니다.
   */
  console.error("이 검사는 빌드 산출물을 읽는다. 검사한 것이 없으므로 통과로 세지 않는다.");
  // **사유는 마지막 줄에 적는다** (2026-08-21). 감사기는 이제 출력 아무 데나가 아니라
  // **마지막 줄**에서 사유를 읽는다 — 중간에 흘린 낱말이 진짜 실패를 「못 돎」으로 덮던
  // 자리를 막은 결과다. 이 순서가 뒤집히면 빌드 안 한 컴퓨터마다 거짓 빨간불이 난다.
