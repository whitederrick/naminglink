...
const nextConfig: NextConfig = {
  ...
  /**
   * **정적 생성 일꾼 수를 묶는다 — 이 앱만 그렇다.**
   *
   * 해몽 사전을 v2로 갈아 끼우자(2026-08-31) 상징이 218개에서 678개가 됐고, 언어판이 23벌
   * 이라 **미리 그리는 장수가 5,014장 → 15,594장(앱 전체 16,153장)** 이 됐다.
   *
   * 그 직후 **드림링크 Vercel 배포가 두 번 연속 실패했다.** 형제 셋은 같은 커밋에서 성공했고,
   * 드림링크의 마지막 성공은 옛 사전을 쓰던 08-27이다. 이 컴퓨터에서도 한 번에 빌드하면
   * 일꾼이 자원 부족으로 죽는다(`build worker exited with code: 3221225794`) — 단계를 나눠
   * 돌리면 끝난다. 일꾼 하나하나가 사전과 렌더러를 통째로 들고 있어 **동시에 일곱이 뜨면
   * 최고 사용량이 그만큼 곱해진다.**
   *
   * 그래서 **장수를 줄이는 대신 동시에 도는 일꾼을 줄인다.** 미리 그리는 범위를 ko·en으로
   * 좁혀도 되지만 그러면 **나머지 21개 언어의 상징 페이지가 404가 된다** — `[locale]/layout.tsx`
   * 의 `dynamicParams = false`가 그 갈래 전체를 지배해서, 잎에서 `dynamicParams = true`로
   * 열어도 되살아나지 않는다(2026-09-01에 실제로 재어 보고 되돌렸다). 23개 언어는 이
   * 서비스의 약속이므로 그쪽을 건드리지 않는다.
   *
   * 빌드는 느려진다. 그 대가로 배포가 선다.
   */
  experimental: {
    cpus: 2,
  },
