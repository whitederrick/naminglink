 ...
 *
 * **글은 로케일 자료가 공지 id로 갖는다.** 예전에는 `notices.ts`가 `{ko, en}` 두 벌을 함께
 * 들고 있었는데, 공지는 **이용 조건이 달라지는 것**을 알리는 자리라 읽지 못하는 언어로 띄우면
 * 알린 것이 되지 않는다. 날짜와 종류만 메타데이터로 남긴 것은 그 값을 스물세 벌로 복제하면
 * 어긋날 수 있고, **어긋난 날짜는 언제 알렸는지를 다투게** 만들기 때문이다.
 *
...
/**
 * **언어는 주소에서, 장 번호는 쿼리에서.** `?page=`는 실제로 내용을 바꾸는 값이라 남긴다 —
 * `?from=`처럼 단추 목적지만 바꾸는 값과 다르다(naminglink `/notice`와 같은 판단이다).
 */
type PageProps = {
    ...
    >
      ...
      {notices.length === 0 ? (
        ...
        shown.map((notice) => {
          ...
          // `scripts/verify-doc-locales.mjs`가 그 빠짐을 센다.
