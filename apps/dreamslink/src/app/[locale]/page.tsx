...
export async function generateMetadata({
  ...
  return {
    // 루트는 template("%s | Dreams-Link")이 브랜드를 또 붙이면 안 되므로 absolute로 둔다.
    ...
    <main className="min-h-screen bg-background">
        ...
        <div className="relative z-10 mx-auto grid w-full max-w-7xl flex-1 gap-5 px-5 pb-4 pt-4 text-white sm:px-8 lg:grid-cols-[minmax(0,1fr)_minmax(34rem,34rem)] lg:px-10 lg:pt-[clamp(6.5rem,14vh,8.5rem)]">
          ...
          <section className="min-w-0 max-w-3xl text-left">
            {/* naminglink 랜딩에는 여기 배지 상자(`copy.badge`)가 있지만 이 화면에서는 뺐다.
                ...
                밀리는 것이 23개 언어에서 가장 큰 불일치 요인이었다.
      ...
      <section className="mx-auto w-full max-w-7xl px-5 py-14 sm:px-8 lg:px-10">
        <div className="max-w-3xl">
          <p className="text-sm font-semibold tracking-wide text-brand-violet">
            {guideDoc.eyebrow}
          </p>
        ...
        {/* 자료 숫자판. 값은 사전·DB에서 오고, 못 읽으면 `DocBody`가 판을 통째로 뺀다. */}
        {statsSection ? (
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-2">
          {guideCards.map((card) => (
            <Link
              ...
              dir={textDirection}
            >
              <p className="text-xs font-semibold tracking-wide text-brand-violet">
                {card.doc.eyebrow}
              </p>
        ...
        <Link
          ...
          className="group mt-3 grid gap-1 rounded-lg border border-brand-violet/25 bg-surface-strong px-5 py-4 transition hover:border-foreground"
          dir={textDirection}
        >
          <p className="text-xs font-semibold tracking-wide text-brand-violet">
            {aboutDoc.eyebrow}
