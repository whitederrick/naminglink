 ...
 * 이동한다. 오픈 전이라 트래픽이 없으니 지금 여는 이득은 없고 잃을 것만 있다.
 *
 * **`NEXT_PUBLIC_SITE_URL`을 실 도메인으로 바꾸면 저절로 열린다.** 따로 기억해 두었다가
 * 꺼야 하는 스위치를 만들지 않은 이유가 그것이다 — 그런 스위치는 잊힌다.
 */
 ...
 * 그 상태였다(`<app>.vercel.app`이 `Allow: /`를 냈다).
 *
 * 주석은 「`*.vercel.app`이면 전면 차단」이라고 적혀 있었는데 **코드는 그렇게 하고 있지
 * 않았다.** 막아야 하는 것은 「어디로 들어왔는가」이므로 요청 호스트를 함께 본다.
 *
 * 검사: `node scripts/verify-robots-open.mjs` — 운영 도메인이 열려 있는지와 배포 주소가
 * 막혀 있는지를 **양쪽 다** 센다.
 */
    ...
    rules: [
      {
        ...
        // 결과 화면은 robots로 막지 않고 화면 쪽에 noindex를 붙였다. robots로 막으면
        // 크롤러가 페이지를 못 읽어 noindex 자체를 못 보게 되고, 외부 링크가 하나라도
        // 있으면 내용 없이 URL만 색인되는 쪽이 오히려 나빠진다.
        disallow: ["/api/"],
      },
    ],
