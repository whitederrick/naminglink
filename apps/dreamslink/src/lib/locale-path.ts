...
/**
 * 로케일이 앞에 붙은 경로를 만든다 — `/ko/compatibility` 꼴.
 *
 ...
 *
 * **화면 코드는 그대로 둔다.** 미들웨어(`proxy.ts`)가 `/ko/compatibility`를
 * `/compatibility?lang=ko`로 되돌려 넘기므로, 각 페이지는 예전처럼 `searchParams.lang`만 읽으면
 * 된다. 라우트 파일을 `[locale]` 아래로 옮기지 않은 이유가 그것이다 — 얻는 것은 같은데
 * 바꿔야 할 코드가 훨씬 많다. naminglink `lib/locale-path.ts`와 같은 처방이다.
 *
 * @param path   `/`로 시작하는 경로. 쿼리는 `query`로 따로 준다.
 * @param locale 없으면 로케일 없는 주소를 돌려준다(헤더로 언어를 정하는 x-default 자리).
 * @param query  함께 실을 쿼리(앞의 `?`는 빼고).
 */
export function localePath(
  ...
  // 경로에 쿼리가 딸려 온 경우를 갈라 둔다. 그대로 앞에 로케일을 붙여도 되지만, 뒤에 쿼리를
  // 더 붙일 때 `?`가 두 번 들어가기 때문이다.
