 ...
 * 아니라 결함으로 읽힌다. 명단은 코어에 있고 여기서 `dreamslink`를 뺀다 — 이 앱을 복제할 때
 * 그 한 글자만 바꾸면 된다.
 */
...
export function SelfAdCard({
  ...
  const candidates = selfAdsExcluding("dreamslink");

  /**
   * **매번 섞는다.** 자리는 둘인데 후보는 넷이라, 고정해 두면 뒤의 둘은 아무에게도 보이지
   * 않는다 — 새 서비스일수록 명단 뒤에 붙는데 그쪽이 알려야 할 쪽이다.
   *
   * 첫 렌더에 난수를 써도 되는 이유: **이 카드는 서버에서 그려지지 않는다.** 지금 이
   * 앱에서 쓰이는 자리는 `AdWatchOverlay` 하나뿐이고, 그건 버튼을 누른 뒤에만 뜬다(서버
   * 렌더를 지나지 않는다).
   *
              ...
              <div className="flex min-w-0 flex-col gap-1">
                ...
                {service.live ? (
                  <a
                    ...
                    className="break-all text-xs text-brand-moonlit underline underline-offset-2"
                  >
                    {service.domain}
                  </a>
                ) : (
