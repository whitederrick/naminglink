...
/**
 * 공개 주소는 `/ko/dream`처럼 **로케일 경로 한 벌**이다.
 *
 * ## 왜 바뀌었나 (2026-08-19)
 *
 * 예전에는 라우트 파일이 평평했고(`app/dream`), 미들웨어가 `/ko/dream`을 `/dream?lang=ko`로
 * **rewrite**했다. 그래서 화면은 `searchParams`를, 루트 레이아웃은 `headers()`를 읽어야 했고,
 ...
 * ## 옛 주소를 끊지 않는다
 *
 * 무접두 주소(`/dream/symbols`)와 `?lang=` 주소는 이미 공유됐고 색인돼 있다. 라우트를 옮기면
 * 그 주소들이 **404가 된다** — 실제로 옮긴 직후 `/`가 404였다. 리다이렉트로 언어판에 모은다.
 *
 *     /                    302 → /{접속 국가·브라우저 언어로 고른 언어판}
 *     /dream/symbols       308 → /en/dream/symbols
 *     /a?lang=ko           301 → /ko/a
