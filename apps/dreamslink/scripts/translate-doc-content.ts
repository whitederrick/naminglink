 ...
 */
const KEEP_VERBATIM = ["Dreams-Link", "dreams-link.com"];
...
const GLOSSARY_ROMAN: Record<string, string> = {
  ...
  행: "elements",
  /**
   * **이 앱의 용어다.** 아래 명리 낱말은 사주링크에서 그대로 복사돼 왔는데, 정작 이 앱이
   * 가장 많이 쓰는 태몽·해몽은 빠져 있었다. 그래서 그 낱말이 남은 잎이 **통째로 영어로
   * 되돌아갔다**(2026-08-10, 다섯 로케일에서 같은 문단).
   *
   * 값은 지어낸 것이 아니라 **이 파일의 프롬프트가 이미 지시하는 뜻** 그대로다.
   */
  태몽: "a conception dream",
  해몽: "dream interpretation",
  길몽: "an auspicious dream",
  흉몽: "an ominous dream",
      ...
      messages: [
        {
          role: "system",
          content: [
            `You translate a Korean dream-interpretation service's help pages into ${language}.`,
            ...
            "  해몽 = dream interpretation in the Korean tradition",
            "  태몽 = a conception dream — a dream believed to foretell a pregnancy",
            "  길몽 = an auspicious dream · 흉몽 = an ominous dream",
            "  상징 = a symbol in the dictionary · 전승 = what the tradition hands down",
            "  사주 = saju (the Korean four-pillars reading) · 한자 = hanja (Chinese characters)",
            ...
            "CRITICAL: write 'Dreams-Link' exactly in Latin letters, never transliterated or translated. It is a trademark and matches the domain dreams-link.com. The same holds for any domain name.",
          ].join("\n"),
        },
