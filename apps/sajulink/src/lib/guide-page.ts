 ...
 *
 * 안내 열 편이 이 한 곳을 거치므로 여기서 요청을 읽으면 **열 편이 한꺼번에** 정적 렌더링에서
 * 빠진다(`lib/route-locale.ts`).
 */
...
/**
 * 안내 문서 열 편이 똑같이 되풀이하던 것을 한곳에 모은다.
 *
 ...
 * `values`도 같다 — 열 문서가 저마다 엔진 상수를 읽으면 어느 하나가 다른 값을 보게 된다.
 */
