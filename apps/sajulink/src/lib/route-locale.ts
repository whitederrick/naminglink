...
import { translated } from "@/lib/locale-detect";
 ...
 *
 * **이 앱은 `translatedLocales`를 쓴다.** 사전이 갖춰진 언어만 담긴 목록이고,
 * `app/sitemap.ts`·`lib/seo.ts`가 이미 같은 목록을 본다 — 셋이 갈리면 sitemap에 있는데
 * 만들어지지 않은 주소나 그 반대가 생긴다. 지금은 23개 전부라 값이 같지만, 번역이 미덥지 않은
 * 언어를 빼는 날 **세 곳이 함께 줄어야** 한다.
 */
