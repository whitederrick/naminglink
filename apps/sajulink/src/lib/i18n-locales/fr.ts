...
export const fr: Dictionary = {
  ...
  "form": {
    ...
    "todayIntro": "La fortune d'aujourd'hui construit les deux caractères du jour — une tige et une branche — de la même manière que votre pilier de naissance a été construit, puis les vérifie par rapport à votre carte natale. Elle additionne et soustrait quatre éléments : si l'énergie d'aujourd'hui correspond à ce dont votre carte a actuellement besoin, comment la tige d'aujourd'hui se rapporte à votre maître du jour en termes des Cinq Éléments, si la branche d'aujourd'hui combine ou s'oppose aux branches de votre carte, et comment une carte forte ou faible reçoit la même énergie différemment — le tout en commençant à partir d'un score de base de 50. La richesse, l'amour, la carrière et la santé héritent chacune d'une partie de ce total et ajoutent leur propre score spécifique, de sorte que deux personnes avec le même score total peuvent encore voir des chiffres différents par domaine.\nChaque poids et limite de grade utilisé ici est publié dans le document de raisonnement ci-dessous.",
