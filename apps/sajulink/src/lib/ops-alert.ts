 ...
 * 제목의 서비스 표시(`[saju-link]`)만 다르다. 한쪽을 고치면 다른 쪽도 함께 볼 것.
 */
...
export function notifyOps(
  ...
  if (webhook) {
    const text = [mark, `[saju-link] ${message}`, body ? "```" + body + "```" : ""]
      .filter(Boolean)
      .join(" ");
    void fetch(webhook, {
      method: "POST",
 ...
 * 인증을 마친 `naming-link.com`에서 보내되 제목에 `[saju-link]`를 달아 어느 서비스인지
...
async function sendAlertEmail(
  ...
  const from =
    ...
    `Saju-Link 알림 <alerts@${VERIFIED_SENDER_DOMAIN}>`;
  ...
  try {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from,
        to,
        subject: `${mark} [saju-link] ${message}`,
        text: body ? `${message}\n\n${body}` : message,
      }),
