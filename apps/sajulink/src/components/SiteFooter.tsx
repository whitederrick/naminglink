...
export async function SiteFooter({
  ...
  className?: string;
  /**
   * 이 화면이 어느 메뉴인가. 푸터의 계산 근거 링크가 이 값을 함께 보내, 안내에서 돌아올 때
   * 원래 화면으로 돌아간다(`lib/guide-back.ts`). 메뉴 화면이 아니면 넘기지 않는다 —
   * 그때는 안내에서 홈으로 나간다.
   *
   * **한 화면에 계산 근거 링크가 둘(본문·푸터) 있다.** 한쪽만 출처를 실으면 같은 말이 적힌
   * 링크 둘이 서로 다른 곳으로 나간다(naminglink에서 실제로 겪었다).
   */
  guideFrom?: "reading" | "today";
}) {
  ...
  const linkClass = isOnDark
    ...
    : "text-foreground hover:text-brand-navy";
