 ...
 * 인연링크 오픈 상태 점검.
 *
 * 인연링크의 잠금은 전부 환경변수로 걸려 있고 그 값은 그쪽 배포 안에만 있다. 이 화면은 그쪽
 * 엔드포인트에 물어 **지금 실제로 켜져 있는 것**을 보여 준다 — Vercel 대시보드에서 값을 눈으로
...
type Payload = { base?: string; siteUrl?: string; checks?: Check[] };
...
export function AdminInyeonStatus() {
  ...
  const load = useCallback(async () => {
    ...
    const response = await fetch("/api/admin/inyeon-status", {
  ...
  return (
    <AdminShell>
      <PageHeader
        title="인연링크 오픈 상태 점검"
        description={
          "인연링크 배포에 직접 물어 지금 켜져 있는 것을 확인합니다. 환경변수 값이 아니라 실제 동작 기준이라, 오타나 재배포 누락도 함께 드러납니다.\n꺼져 있는 항목이 있는 것이 곧 문제는 아닙니다 — 아직 오픈 전이라면 대부분 꺼져 있는 것이 정상입니다."
        }
      >
        <button
          type="button"
      ...
      {loading ? (
        <Empty>인연링크 배포에 확인하는 중입니다.</Empty>
      ) : checks.length === 0 ? null : (
        <>
          <p className="mb-4 text-sm text-muted">
            대상 <b className="text-foreground">{payload?.siteUrl ?? payload?.base}</b> · 켜진 항목{" "}
          ...
          <Table
            ...
            rows={checks.map((check) => [
              <div key="label">
                ...
                {check.note ? <p className="mt-0.5 text-xs text-muted">{check.note}</p> : null}
              </div>,
              <span
                key="state"
