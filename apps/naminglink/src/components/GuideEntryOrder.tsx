 ...
 *
 * ## 왜 브라우저에서 하나 (2026-08-18)
 *
 * 허브는 온 곳을 알면 그 갈래의 문서를 앞에 놓는다(`guideEntriesFor`). 그 값이 쿼리라서
 * **서버가 읽는 순간 허브 전체가 미리 만들어지지 못한다.** 23개 언어판 허브가 요청마다
 * 새로 그려지고 있었다.
 *
...
export function GuideEntryOrder({ orders, className, children }: Props) {
  return (
    <Suspense fallback={<nav className={className}>{children}</nav>}>
