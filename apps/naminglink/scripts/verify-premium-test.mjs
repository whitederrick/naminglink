...
//
// AUDIT_SIDE_EFFECTS: dev 서버의 프리미엄 경로를 실제로 태운다 — OpenAI 토큰 비용이 든다
//
// **힌트로는 못 잡히는 갈래다** (2026-08-21). 감사기의 부수효과 힌트는 소스에서 `openai`
// 임포트나 DB 쓰기를 찾는데, 이 파일은 **HTTP 로 우리 앱을 때려서** 비용을 낸다. 임포트가
// 없으니 정규식에 안 걸린다. 힌트는 그물이지 울타리가 아니다 — 아는 사람이 적어야 한다.
//
