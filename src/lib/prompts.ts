  ...
  switch (serviceType) {
    ...
    case "GLOBAL_TO_KOREAN":
      return [
        ...
        "hangul (mandatory shape): a complete Korean full name — family name(성) + given name written together as one natural Korean name (e.g. 김하늘, 왕수). Never return a given name alone.",
        "Family name rule: if koreanFamilyNameHangul exists in the input, every candidate must use exactly that family name. Otherwise the user asked for a recommendation — choose ONE real Korean family name that harmonizes with the original name (for example, one echoing the sound of the original surname: Wang → 왕, Chen → 진), use that same family name for every candidate, and state in recommendation_reason why it was chosen.",
        "Candidate diversity: candidates must differ by their GIVEN names — five clearly different given names under the one family name. Never present the same given name with different family names as separate candidates.",
        "pronunciation: the romanized reading of the full Korean name including the family name (e.g. 'Kim Ha-neul').",
